...
const registerUser = async (req, res) => {
  try {
    ...
    const reqPassword = typeof req.body.password === 'string' ? req.body.password.trim() : '';
    const busId = req.body.busId || null;
    const stopSeq = req.body.stopSeq != null ? Number(req.body.stopSeq) : null;

    if (!username) {
      return res.status(400).json({ message: 'Passenger ID / Username is required' });
    }
    if (!email) {
    ...
    if (existingUser) {
      return res.status(409).json({ message: 'Passenger ID already registered' });
    }
    ...
    // Password is auto-set to the roll number (username) if not provided by user
    const passwordToHash = reqPassword || username;
    ...
    const user = await User.create({
      ...
      name,
      email,
      firstLogin: !reqPassword // Force password change on first login only if auto-generated
    });
    ...
    // Send welcome email with credentials asynchronously if auto-generated
    if (!reqPassword) {
      sendWelcomeEmail({
        email: user.email,
        fullName: user.name,
