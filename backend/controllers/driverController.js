...
const recordStopEvent = async (req, res) => {
  ...
  if (trip.route?.stops?.length > 0) {
    orderedStops = [...trip.route.stops].sort((a, b) => (a.seq || 0) - (b.seq || 0));
  } else {
    orderedStops = await Stop.find({ route: trip.route }).sort({ sequence: 1 });
  }

  const stopIndex = orderedStops.findIndex(
    (item) => item._id.toString() === stopId || String(item.seq) === String(stopId)
  );
  ...
  if (status === 'ARRIVED') {
    await sendPushNotification({
      busId: trip.bus,
      title: 'Bus Arrived',
      body: `Your bus has arrived at ${stop.name}. Please board now!`
    });
  } else if (status === 'LEFT') {
    let bodyText = `The bus has left ${stop.name}.`;
    if (trip.currentStopIndex < orderedStops.length) {
      const nextStop = orderedStops[trip.currentStopIndex];
      bodyText = `Next Stop: ${nextStop.name}`;
    }
