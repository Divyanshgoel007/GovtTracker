            ...
            (async () => {
              try {
                ...
                for (const assignment of studentsWithPush) {
                  ...
                  const upcomingStopName = (state.routeStops && nextStopIndex < state.routeStops.length)
                    ? state.routeStops[nextStopIndex].name
                    : 'End of Route';
                  ...
                  const notifBody = isTheirStop
                    ...
                    : `Bus arrived at ${event.stopName}. Next stop is ${upcomingStopName}.`;
...
const handleManualEvent = async (io, socket, payload) => {
  ...
  // --- ADD PUSH NOTIFICATIONS FOR MANUAL EVENTS ---
  (async () => {
    try {
      const StudentAssignment = require('../models/StudentAssignment');
      const { sendPush } = require('./notificationController');
      ...
      const assignments = await StudentAssignment.find({
        bus: state.trip.bus,
        'notificationPreferences.enabled': true,
        ...(status === 'ARRIVED' && { 'notificationPreferences.arrivalAlert': true })
      }).populate('student').populate('stop', 'sequence name');

      const studentsWithPush = assignments.filter(a => a.student?.pushSubscription);

      if (studentsWithPush.length > 0) {
        if (status === 'ARRIVED') {
          const nextStopIndex = state.currentStopIndex + 1;
          const upcomingStopName = (state.routeStops && nextStopIndex < state.routeStops.length)
            ? state.routeStops[nextStopIndex].name
          ...
          for (const assignment of studentsWithPush) {
            const stu = assignment.student;
            const isTheirStop = assignment.stop?.sequence === stopIndex;
            const notifBody = isTheirStop
              ? `🎯 Bus has arrived at ${event.stopName} - YOUR STOP!`
              : `Bus arrived at ${event.stopName}. Next stop is ${upcomingStopName}.`;
            ...
            await sendPush(stu, {
              ...
              url: '/student',
              tag: 'stop-arrival',
              requireInteraction: isTheirStop
            });
          }
        } else if (status === 'LEFT') {
          for (const assignment of studentsWithPush) {
      ...
      console.error('Manual Event Push Error:', pushErr.message);
    }
  })();
};
