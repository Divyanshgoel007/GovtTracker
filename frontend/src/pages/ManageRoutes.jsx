              ...
              <div className="rw-routes-header">
                ...
                <div className="flex items-center gap-2">
                  <button 
                    onClick={async () => {
                      try {
                        const toastId = toast.loading('Importing from API...');
                        const res = await api.post('/routes/import-demo');
                        toast.success(`Imported ${res.data.imported.length} routes from API`, { id: toastId });
                        loadRoutes();
                      } catch (err) {
                        toast.error(err.response?.data?.message || 'Failed to import routes from API');
                      }
                    }}
                    className="rw-btn rw-btn-primary text-xs py-1 px-3 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700"
                    title="Generate demo routes using API Key"
                  >
                    Import City Routes (API)
                  </button>
