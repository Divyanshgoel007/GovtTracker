...
const VAPID_PUBLIC_KEY = import.meta.env.VITE_VAPID_PUBLIC_KEY || 'BAtGiwcOhuOVlxgLFG2wuAc6HD8XEYqTEunc00Ve4rAXV4yoV9VjQQsCYmcOdrepF5RWPRZ9HQS4iDYw60rscvg';
...
const BusSelectorModal = ({ isOpen, onClose, onSave, buses }) => {
  ...
  const currentBus = buses.find(b => b._id === selectedBus);
  const stops = currentBus?.route?.stops || [];

  const handleSave = async () => {
    if (!selectedBus || !selectedStop) return;
    setLoading(true);
    await onSave(selectedBus, parseInt(selectedStop, 10));
    setLoading(false);
  };
  ...
  return (
    <div className="sd-modal-overlay">
      <div className="sd-modal">
        <h2 className="text-xl font-bold text-white mb-2">Select Your Bus</h2>
        <p className="text-sm text-slate-400 mb-6">Choose the bus and stop you want to track.</p>

        <div className="space-y-4 mb-6">
          <div>
            <label className="sd-settings-label">Available Buses</label>
            <select
              className="w-full bg-slate-900/80 border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-orange-500/50"
              value={selectedBus}
              onChange={(e) => {
          ...
          {selectedBus && stops.length > 0 && (
            <div className="animate-fade-in">
              <label className="sd-settings-label">Your Stop</label>
              <select
                className="w-full bg-slate-900/80 border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-orange-500/50"
                value={selectedStop}
              ...
              >
                ...
                {stops.map(s => (
                  <option key={s._id} value={s.seq}>{s.name}</option>
                ))}
              </select>
            </div>
          )}
          {selectedBus && stops.length === 0 && (
            <p className="text-xs text-red-400">This bus has no route stops assigned.</p>
          )}
        </div>

        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 sd-btn-outline py-2.5">
            Cancel
          </button>
          <button
            onClick={handleSave}
            disabled={!selectedBus || selectedStop === '' || loading}
            className="flex-1 sd-btn-primary py-2.5 disabled:opacity-50"
          >
            {loading ? 'Saving...' : 'Track Bus'}
          </button>
        </div>
      </div>
    </div>
...
const StudentDashboard = () => {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      // If no assignment, pop open the bus selector automatically
      if (!data?.bus) {
         fetchAvailableBuses();
      ...
      if (tripRes.data) {
        ...
        setRouteGeojson(tripRes.data.route?.geojson || null);
        setRouteStops(tripRes.data.route?.stops || []);
      ...
      if (tripRes.data) {
        ...
        if (etaRes.data?.allEtas) {
          setAllEtas(etaRes.data.allEtas);
        }
      }
    } catch (err) {
  ...
  const fetchAvailableBuses = async () => {
    try {
      const res = await api.get('/students/buses');
  ...
  const handleUpdateAssignment = async (busId, stopSeq) => {
    try {
      await api.put('/students/assignment', { busId, stopSeq });
      toast.success('Bus selected!');
  ...
  return (
    <main className="sd-page">
      ...
      <BusSelectorModal
        ...
        onClose={() => {
          if (!profile?.bus) toast.error('You must select a bus to track');
          else setShowBusSelector(false);
        }}
      ...
      <div className="sd-container">
        ...
        <header className="sd-header sd-card-animate">
          ...
          <div className="sd-header-status">
            ...
            <button 
              onClick={() => { fetchAvailableBuses(); setShowBusSelector(true); }}
              className="text-xs bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-full transition-colors flex items-center gap-1 ml-2"
            >
              <Bus className="w-3 h-3" /> Change Bus
            </button>
            {lastRefreshed && (
            ...
            <div className="sd-map-wrap sd-card-animate" style={{ animationDelay: '0.12s' }}>
              <div className="sd-map-inner">
                <StudentMap 
                  ...
                  allEtas={allEtas}
                  studentStopSeq={stopInfo?.seq ?? stopInfo?.sequence}
                />
              </div>
            </div>
