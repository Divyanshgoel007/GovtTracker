...
const DriverDashboard = () => {
  ...
  const handleStopEvent = async (status) => {
    if (!trip || !trip.route?.stops) return;
    const currentStop = trip.route.stops[trip.currentStopIndex || 0];
    if (!currentStop) return;

    try {
      await api.post('/trips/event', {
        tripId: trip._id || trip.id,
        stopId: currentStop._id,
        status
      });

      setStatusMessage(`Marked ${status} at ${currentStop.name}`);
      addLog(`🔘 ${status}: ${currentStop.name}`);

      // Refresh trip to get updated currentStopIndex
      const { data } = await api.get('/trips/active');
      if (data?._id) setTrip(data);
    } catch (err) {
      setStatusMessage(err.response?.data?.message || 'Failed to update stop status');
    }
  };
                ...
                {!trip ? (
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="bg-slate-900/50 border border-slate-700 rounded-xl p-4">
                      <div className="text-sm text-slate-400 mb-2">Next Stop:</div>
                      <div className="text-lg font-bold text-white mb-4">
                        {trip.route?.stops && trip.route.stops[trip.currentStopIndex || 0] 
                          ? trip.route.stops[trip.currentStopIndex || 0].name 
                          : 'Route Complete!'}
                      </div>
                      
                      <div className="dd-trip-actions-split">
                        <button 
                          onClick={() => handleStopEvent('ARRIVED')} 
                          className="dd-btn bg-green-600/20 text-green-400 hover:bg-green-600/30 border border-green-600/30"
                          disabled={!trip.route?.stops || trip.currentStopIndex >= trip.route.stops.length}
                        >
                          <MapPin className="w-5 h-5" />
                          Arrive
                        </button>
                        <button 
                          onClick={() => handleStopEvent('LEFT')} 
                          className="dd-btn bg-orange-600/20 text-orange-400 hover:bg-orange-600/30 border border-orange-600/30"
                          disabled={!trip.route?.stops || trip.currentStopIndex >= trip.route.stops.length}
                        >
                          <Navigation className="w-5 h-5" />
                          Depart
                    ...
                    <div className="dd-trip-actions-split mt-2">
