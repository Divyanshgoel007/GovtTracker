...
const AssignStudents = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Unable to assign passenger');
    }
  };
    ...
    <main className="min-h-screen pb-24 md:pb-8">
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Passenger Assignments</h1>
            <p className="text-sm text-slate-400 mt-1">Assign passengers to buses and stops</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          <StatCard icon={CheckCircle} label="Assigned" value={assignments.length} subtitle="Passengers" color="emerald" />
      ...
      <Drawer
        isOpen={drawerOpen}
        title="Edit Assignment"
        subtitle="Change bus or stop for this passenger"
