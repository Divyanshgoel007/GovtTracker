...
const Navbar = () => {
  ...
  const adminLinks = [
    ...
    { to: '/admin/students', icon: Users, label: 'Passengers' },
  ...
  const getAllLinks = () => {
    ...
    if (user?.role === 'student' || user?.role === 'passenger') return [
  ...
  const getMobileNav = () => {
    if (user?.role === 'student' || user?.role === 'passenger') {
      return [
  ...
  return (
    <>
      ...
      {/* ===== Mobile Bottom Navigation (Passenger/Driver only) ===== */}
      {user && (user.role === 'student' || user.role === 'passenger' || user.role === 'driver') && mobileNav.length > 0 && (
